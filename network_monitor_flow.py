...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def show_stat(self, type, bodys):
        ...
        if(type == 'flow'):
            ...
            for dpid in bodys.keys():
                for stat in sorted(
                    ...
                    print('%s %8s %23s %6s %8s %8s %8.1f' % (
                  #  print('%016x %8x %17s %s %8d %8d %8.1f' % (
                        dpid,
        ...
        if(type == 'port'):
            ...
            print('--------  -------- ''-------- -------- -------- '
                  ...
                  '----------------  ----------------   '
                  '   -----------    -----------')
            format = '%s %8x %8d %8d %8d %8d %8d %8d %8.1f %16d %16s %16s'
            for dpid in bodys.keys():
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if flow.priority == 1],
                           key=lambda flow: (flow.match.get('in_port'),
                                             flow.match.get('eth_dst'))):
            key = (stat.match.get('in_port'),  stat.match.get('eth_dst'),
                   stat.instructions[0].actions[0].port)
        ...
        for stat in sorted(body, key=attrgetter('port_no')):
            if stat.port_no != ofproto_v1_3.OFPP_LOCAL:
                ...
                speed = self._get_speed(
                    ...
                    pre, period)
                upload = self._get_speed( self.port_stats[key][-1][0] ,
                    pre_up, period)
                download = self._get_speed(self.port_stats[key][-1][1],
                                         pre_down, period)

                self._save_stats(self.port_upload,key, upload,5 )
                self._save_stats(self.port_download, key, download, 5)
